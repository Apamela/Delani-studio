$(document).ready(function () {
    $("#mella").click(function () {
        $("#mell").toggle();
        $("#mella").toggle();
    });
    $("#mell").click(function () {
        $("#mella").toggle();
        $("#mell").toggle();
    });
    $("#pella").click(function () {
        $("#pell").toggle();
        $("#pella").toggle();
    });
    $("#pell").click(function () {
        $("#pella").toggle();
        $("#pell").toggle();
    });
    $("#patte").click(function () {
        $("#patt").toggle();
        $("#patte").toggle();
    });
    $("#patt").click(function () {
        $("#patte").toggle();
        $("#patt").toggle();
    });
    $("#cena").mouseover(function () {
        $("#comp").show();
    });
    $("#cena").mouseleave(function () {
        $("#comp").hide();
    });
    $("#son").mouseover(function () {
        $("#kell").show();
    });
    $("#son").mouseleave(function () {
        $("#kell").hide();
    });
    $("#sen").mouseover(function () {
        $("#mim").show();
    });
    $("#sen").mouseleave(function () {
        $("#mim").hide();
    });
    $("#pam").mouseover(function () {
        $("#mom").show();
    });
    $("#pam").mouseleave(function () {
        $("#mom").hide();
    });
    $("#lolo").mouseover(function () {
        $("#receive").show();
    });
    $("#lolo").mouseleave(function () {
        $("#receive").hide();
    });
    $("#up").mouseover(function () {
        $("#send").show();
    });
    $("#up").mouseleave(function () {
        $("#send").hide();
    });
    $("#sendy").mouseover(function () {
        $("#oll").show();
    });
    $("#sendy").mouseleave(function () {
        $("#oll").hide();
    });
    $("#pop").mouseover(function () {
        $("#lli").show();
    });
    $("#pop").mouseleave(function () {
        $("#lli").hide();
    });
    $("#yes").click(function () {
        var name = ("#fname").val();
        var email = ("#fname").val();
        var message = ("#pname").val();
        if (name == "" || email == "" || message == "") {
            alert("you have to fill this requirement!");
        }
        else {
            alert(name + "we have received your message".thank);
        }

    });

});
